...
func (r *Reconciler) handleAutoscalerDeploy(ctx context.Context, cfg membercfg.Configuration, namespace string) error {
	...
	if cfg.Autoscaler().Deploy() {
		...
		if err := autoscaler.Deploy(ctx, r.Client, r.Client.Scheme(), namespace, cfg.Autoscaler().BufferMemory(), cfg.Autoscaler().BufferCPU(), cfg.Autoscaler().BufferReplicas()); err != nil {
			return err
		}
